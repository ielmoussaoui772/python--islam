...
def leer_csv(csv_file):
    ...
    with open(csv_file, 'r') as file:
        ...
        for row in reader:
            ...
            full_name = f"{first_name} {last_name}"  # Se agrega un espacio entre el primer nombre y el apellido
...
def crear_usuarios(csv_users):
    ...
    for user in csv_users:
        ...
        if result.returncode == 0:
            # Si el usuario ya existe, se escribe en el log
            with open('usuarirepetit.log', 'a') as log_file:
                log_file.write(f'El usuario {user} ya está creado.\n')
        else:
